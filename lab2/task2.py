...
def print_iteration_history(title: str, history):
    ...
    for k, x1k, x2k, step, estimate, residual in history:
        print(f"{k:4d} | {x1k:17.12f} | {x2k:17.12f} | {step:17.10e} | {estimate:17.10e} | {residual:10.3e}")
...
def main():
    ...
    print("x2 - sin(x1) = a")
    print(f"a = {a}")
    print(f"Графическая локализация положительного решения: x1 in {X1_INTERVAL}, x2 in {X2_INTERVAL}")
    print(
        "Проверка интервала для простой итерации: "
        f"phi1 subset [{phi1_range[0]:.6f}, {phi1_range[1]:.6f}], "
        f"phi2 subset [{phi2_range[0]:.6f}, {phi2_range[1]:.6f}], q = {q:.6f}"
    )
    print(f"Начальное приближение (графически): x1_0 = {x1_0}, x2_0 = {x2_0}")
    print(f"Точность: eps = {eps}\n")

    (x1_it, x2_it), hist_it = simple_iteration(a, x1_0, x2_0, eps, q)
    (x1_n, x2_n), hist_n = newton(a, x1_0, x2_0, eps)

    print_iteration_history("Метод простой итерации:", hist_it)
    r1_it = f1(x1_it, x2_it, a)
    r2_it = f2(x1_it, x2_it, a)
    print(f"Результат: x1 = {x1_it:.12f}, x2 = {x2_it:.12f}, невязка = {max(abs(r1_it), abs(r2_it)):.3e}")

    print_newton_history("Метод Ньютона:", hist_n)
    r1_n = f1(x1_n, x2_n, a)
    r2_n = f2(x1_n, x2_n, a)
    print(f"Результат: x1 = {x1_n:.12f}, x2 = {x2_n:.12f}, невязка = {max(abs(r1_n), abs(r2_n)):.3e}")
